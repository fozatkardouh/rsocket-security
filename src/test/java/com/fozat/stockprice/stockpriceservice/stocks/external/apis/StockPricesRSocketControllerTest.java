...
class StockPricesRSocketControllerTest {
    ...
    public static void setupOnce(
            @Autowired RSocketRequester.Builder builder, @LocalRSocketServerPort Integer port) {
    ...
    public void testStreamGetsStream() {
        ...
        StockPriceRSocketRequest request1 = StockPriceRSocketRequest.builder().interval(2).stockName("TEST1").build();
        Mono<StockPriceRSocketRequest> setting1 = Mono.just(request1).delayElement(Duration.ofSeconds(0));

        // Create next setting after 3 seconds. Server starts sending in after 1 second.
        StockPriceRSocketRequest request2 = StockPriceRSocketRequest.builder().interval(1).stockName("TEST2").build();
        Mono<StockPriceRSocketRequest> setting2 = Mono.just(request2).delayElement(Duration.ofSeconds(3));
        ...
        Flux<StockPrice> stream = requester.route("stockPrices").data(settings).retrieveFlux(StockPrice.class);

        // Verify that the response messages contain the expected data
        StepVerifier.create(stream).consumeNextWith(stockPrice -> {
            assertThat(stockPrice.getSymbol()).isEqualTo("TEST1");
            assertThat(stockPrice.getPrice()).isBetween(0d, 100d);
        }).expectNextCount(0).consumeNextWith(stockPrice -> {
            assertThat(stockPrice.getSymbol()).isEqualTo("TEST2");
        }).thenCancel().verify();
    }
